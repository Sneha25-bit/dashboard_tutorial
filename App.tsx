...
const App: React.FC = () => {
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex-1">
            ...
            <p className="text-blue-100 text-sm italic max-w-2xl leading-relaxed">
              {isAnalyzing ? "Gemini is scanning your performance data..." : aiAnalysis || "Click refresh to analyze your current standing."}
            </p>
          </div>
          <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          { label: 'Pending Tasks', value: '4', sub: '2 due this week', color: 'amber', icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z' },
              ...
              {MOCK_SUBJECTS.map((sub) => (
                <div key={sub.id} className="p-4 rounded-xl border border-slate-100 bg-slate-50/50">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <span className={`px-2 py-1 rounded-lg text-[10px] font-bold uppercase ${sub.attendance >= 75 ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                      {sub.attendance}%
                    </span>
                  </div>
  ...
  const renderAttendance = () => (
    <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Detailed Attendance</h2>
          <p className="text-slate-500 mt-1">SOP Rule: Minimum 75% required to avoid penalties.</p>
        </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-100 text-left">
              ...
              <th className="pb-4 font-semibold text-slate-400 text-xs uppercase tracking-wider">Percentage</th>
              <th className="pb-4 font-semibold text-slate-400 text-xs uppercase tracking-wider">Indicator</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-50">
            {MOCK_SUBJECTS.map((sub) => (
              <tr key={sub.id} className="group hover:bg-slate-50 transition-colors">
                <td className="py-4 font-semibold text-slate-700">{sub.name} <span className="text-xs font-normal text-slate-400 block">{sub.code}</span></td>
                ...
                <td className="py-4 font-bold text-slate-800">{sub.attendance}%</td>
                <td className="py-4">
        ...
        <div className="flex-1">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={MOCK_PERFORMANCE}>
              ...
              <Tooltip />
              <Line type="monotone" dataKey="sgpa" stroke={COLORS.primary} strokeWidth={4} dot={{ r: 6, fill: COLORS.primary }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
  ...
  const renderAssignments = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in zoom-in-95 duration-300">
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <h3 className="text-lg font-bold text-slate-800 mb-6 flex items-center justify-between">Pending Tasks <span className="text-xs bg-amber-100 text-amber-700 px-2 py-1 rounded-lg">Action Required</span></h3>
        <div className="space-y-4">
          {MOCK_ASSIGNMENTS.filter(a => a.status === 'pending').map((item) => (
            <div key={item.id} className="p-4 rounded-xl border border-slate-100 hover:border-blue-200 transition-colors bg-slate-50/50">
              ...
              <div className="flex justify-between items-center text-[10px] font-bold text-rose-500 uppercase">
                ...
                <span className="bg-rose-50 px-2 py-0.5 rounded">Weight: {item.weightage}%</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <h3 className="text-lg font-bold text-slate-800 mb-6">Completed</h3>
        <div className="space-y-4">
          {MOCK_ASSIGNMENTS.filter(a => a.status === 'submitted').map((item) => (
            <div key={item.id} className="p-4 rounded-xl bg-emerald-50/30 border border-emerald-100 flex items-center justify-between">
              <div>
  ...
  const renderCalculator = () => (
    <div className="max-w-xl mx-auto bg-white p-10 rounded-3xl border border-slate-200 shadow-lg animate-in scale-95 duration-500">
      <h2 className="text-2xl font-bold text-slate-900 mb-2">CGPA What-If Tool</h2>
      <p className="text-slate-500 mb-8">Estimate your potential CGPA based on future performance targets.</p>
      <div className="space-y-6">
        <div><label className="text-sm font-bold text-slate-700 block mb-2 uppercase">Current CGPA</label><input type="number" defaultValue="8.18" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none" /></div>
        <div><label className="text-sm font-bold text-slate-700 block mb-2 uppercase">Target CGPA</label><input type="number" defaultValue="8.5" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none" /></div>
        <button className="w-full py-4 bg-blue-600 text-white rounded-2xl font-bold hover:bg-blue-700 transition-shadow shadow-md">Run Prediction</button>
        <div className="mt-8 p-6 bg-blue-50 rounded-2xl border border-blue-100 text-center"><p className="text-xs text-blue-500 uppercase font-bold tracking-widest mb-1">Required Avg SGPA</p><p className="text-4xl font-black text-blue-800">8.82</p></div>
      </div>
    </div>
  );
  ...
  const renderChecklist = () => (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 animate-in fade-in duration-500">
      {[
        { title: 'Daily', color: 'blue', items: ['Attendance check', 'Remark review', 'Upcoming deadlines', 'Topic status update'] },
        { title: 'Weekly', color: 'indigo', items: ['Topic comprehension', 'Attendance planning', 'Study block schedule', 'Priority assignment check'] },
        { title: 'Monthly', color: 'purple', items: ['CGPA trajectory', 'Goal assessment', 'Advisor consultation', 'Monthly growth log'] },
      ].map((list, idx) => (
        <div key={idx} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:border-blue-300 transition-all">
          <h3 className={`text-${list.color}-600 font-bold mb-6 text-sm uppercase tracking-widest`}>{list.title} Routine</h3>
          <div className="space-y-4">
      ...
      <main className="flex-1 ml-64 min-w-0">
        ...
        <div className="p-8 max-w-7xl mx-auto h-[calc(100vh-4rem)] overflow-y-auto custom-scrollbar">
          <div className="mb-8 flex justify-between items-end">
            <div>
              <p className="text-sm text-slate-500 font-medium italic">Empowering SVNIT Scholars</p>
              <h1 className="text-3xl font-bold text-slate-900">Student Dashboard</h1>
            </div>
            <div className="bg-white p-2 rounded-2xl border border-slate-200 flex space-x-2 text-xs font-bold text-slate-500 shadow-sm">
              <span className="px-4 py-2 bg-slate-50 text-blue-600 rounded-xl border border-slate-100">B.Tech CS - Sem 4</span>
              <span className="px-4 py-2 rounded-xl border border-slate-100 hover:bg-slate-50 cursor-pointer">Academic Year 2024-25</span>
            </div>
          </div>
          {renderContent()}
        </div>
